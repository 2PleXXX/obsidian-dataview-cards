...
const matchFactories = {
  matchBySectionValue: (config, args) => (page) => {
    ...
    const pageValue = String(page[field] ?? "").trim();
  ...
  async function checkForScriptUpdates(dv, t) {
    ...
    try {
      ...
      if (
        ...
        notice.innerHTML = `🆕 ${updateText} &nbsp; <a href="${GITHUB_REPOSITORY}" target="_blank">${updateLinkText}</a>`;
        versionContainer.appendChild(notice);
      }
    } catch (e) {
      console.error(
        `💥 ${t?.UPDATES?.UPDATE_CHECK_ERROR || "Update check failed"}: ${
          e.message
        }`
      );
    }
